...
# At the moment, the RenderCallback class only works with MultiInput Policy agents
# Check the RenderCallback class episode lengths count functionality
...
class RenderCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if "done" in self.locals and self.locals["done"]:
            self.episode_len_list.append(
                self.trajectory_len
            )  # Append the step count for this episode
            self.trajectory_len = 0  # Reset the step counter for the next episode
        else:
            self.trajectory_len += 1  # Increment the step counter
    ...
    def _on_training_end(self) -> None:
        ...
        self.episode_len_list = []  # Clean up the episode length list
...
class VQVAE_Env(gym.Env):
    ...
    def render(self, mode="human"):
        if mode == "human":
            ...
            num_trajectories = len(all_states_2d) // RENDER_STEPS
            ...
            for i in range(num_trajectories):
                ...
                states_2d = all_states_2d[
                    i * RENDER_STEPS : i * RENDER_STEPS + RENDER_STEPS
                ]

                start = 0
                color_map = cm.get_cmap(
                    "viridis", len(ep_len_states)
                )  # You can choose any available colormap

                for index, length in enumerate(ep_len_states):
                    plt.plot(
                        states_2d[start : start + length, 0],
                        states_2d[start : start + length, 1],
                        color=color_map(
                            index
                    ...
                    # Plot the start point with a specific marker
                    plt.plot(
                        states_2d[start, 0],
                        states_2d[start, 1],
                        color="green",  # Color for the start point
                    ...
                    # Plot the end point with a different marker
                    plt.plot(
                        states_2d[start + length - 1, 0],
                        states_2d[start + length - 1, 1],
                        color="red",  # Color for the end point
                    ...
                    start += length  # Update start position for the next episode
                ...
                print(f"{self.log_dir}/latent_space_{timestamp_str}.png")
